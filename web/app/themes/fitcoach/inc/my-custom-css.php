...
Author: Salvatore Noschese - DarkWolf and Cliff Seal
Version: 1.5
...
{
global $blog_id; $cssid = ( $blog_id > "1" ) ? $cssid = "_id-".$blog_id : $cssid = null;
$css_url = get_template_directory_uri()."/inc/my_style".$cssid.".css";
return $css_url;
}
function icon_link()
{
...
{
# Settings:
$settings_link = '<a href="admin.php?page=my_custom_css" title="My Custom CSS">'.__('Settings','fitcoach').'</a>';
array_unshift($links, $settings_link);
# Support:
$support_forum = '<a href="http://www.darkwolf.it/" title="DarkWolf">'.__('Support','fitcoach').'</a>';
...
{
$plugin_page = add_theme_page(__('My Custom CSS Panel','fitcoach'),__('Custom CSS','fitcoach'), 'manage_options', 'my_custom_css', 'mccss_options', icon_link());
...
{
$plugdir = get_template_directory_uri();
?>
<!-- Syntax Support Start -->
<link type="text/css" rel="stylesheet" href="<?php echo $plugdir; ?>/css/codemirror.css" />
<script language="javascript" src="<?php echo $plugdir; ?>/js/codemirror.js"></script>
<script language="javascript" src="<?php echo $plugdir; ?>/js/css.js"></script>
<!-- Syntax Support End -->
<?php
}
// Register Settings
...
?>
<div class="wrap">
<h2><?php _e('Custom CSS Panel','fitcoach'); ?></h2>
<form method="post" action="options.php">
<?php settings_fields( 'mccss_settings' ); ?>
<p><?php _e('Place your custom CSS code here:','fitcoach'); ?> <input type="submit" class="button-secondary" value="<?php _e('Save','fitcoach'); ?>" style="font-size: 20px; height: 35px; position: fixed; top: 35px; right: 15px; z-index: 5;" /></p>
...
<p>
<input type="submit" class="button-primary" value="<?php _e('Save','fitcoach'); ?>" />
